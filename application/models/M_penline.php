...
class M_penline extends CI_Model
{
  ...
  {
    $this->db->select("dsosys_status_kawin.NAMA as kawin, dsosys_agama.NAMA as agama, dsosys_pendidikan.NAMA as pendidikan, dsosys_pekerjaan.NAMA as pekerjaan, dsosys_kota.NAMA as kota, dsosys_provinsi.NAMA as prov, dsosys_pendidikan_kk.NAMA as pendidikan_kk, dsosys_status_kewarganegaraan.NAMA as kewar");
    $this->db->from("dsosys_pendaftaran_online as online");
    $this->db->join('dsosys_status_kawin', 'dsosys_status_kawin.ID = online.ID_STATUS_KAWIN');
    $this->db->join('dsosys_agama', 'dsosys_agama.ID = online.ID_AGAMA');
    $this->db->join('dsosys_pendidikan', 'dsosys_pendidikan.ID = online.ID_PENDIDIKAN_SEDANG');
    $this->db->join('dsosys_pekerjaan', 'dsosys_pekerjaan.ID = online.ID_PEKERJAAN');
    $this->db->join('dsosys_provinsi', 'dsosys_provinsi.ID = online.ID_PROVINSI');
    $this->db->join('dsosys_kota', 'dsosys_kota.ID = online.ID_KABNKOTA');
    $this->db->join('dsosys_pendidikan_kk', 'dsosys_pendidikan_kk.ID = online.ID_PENDIDIKAN_KK');
    $this->db->join('dsosys_status_kewarganegaraan', 'dsosys_status_kewarganegaraan.ID = online.ID_KEWARGANEGARAAN');
    $this->db->where('online.ID', $id);
    return $this->db->get()->row();
  }
  ...
  public function datatablePenline()
  {
    ...
    foreach ($q as $val) {      
      $check = '<input type="checkbox" id="'.$val->ID.'" class="checkitem" name="cek[]" onclick="la(\'' .$val->ID. '\')" />' ;
      ...
      <a href="#" data-url="'.site_url('P_online/penline_delete/'.$val->ID).'" class="btn btn-danger btn-delete btn-xs" 
      style="text-align: center; width: 37px;" 
      data-toggle="tooltip" title="hapus"><i class="fa fa-trash"></i>
      </a>';

      if ($val->STATUS_ACC == "1") {
        $btn .= ' <a OnClick="acc('.$val->ID.', \''.$val->NAMA_LENGKAP.'\')" class="btn btn-success btn-xs" 
        style="text-align: center; width: 37px; color: white" 
        data-toggle="tooltip" title="setujui"><i class="mdi mdi-account-plus"></i>
        </a>';
      }

      $statusacc = $val->STATUS_ACC;

      if ($val->STATUS_ACC == "1"){
        $statusacc = '<label class="badge badge-danger">belum disetujui</label>';
      }else{
        $statusacc = '<label class="badge badge-success">disetujui</label>';
      }
      ...
      if ($val->CREATED_AT){
        $tgldaftar = date('d/m/Y - H:i',strtotime($val->CREATED_AT));
      }
