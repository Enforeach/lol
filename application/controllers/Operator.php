...
class Operator extends Core {
	...
	public function addOperator()
	{
		...
		public function Operator_delete ($nama_s)
		{
			...
			if (file_exists("./images/operator/".$this->m_operator->getFoto($nama_s, 'FOTO'))) {
				unlink("./images/operator/".$this->m_operator->getFoto($nama_s, 'FOTO'));
			}
		...
		public function del_bulk ()
		{
			...
			for ($i=0; $i < count($data) ; $i++) { 
				...
				$where = array('NAMA_SINGKAT' => $data[$i]);
				$this->m_operator->Operator_delete($where,'dsosys_operator');

				if (file_exists("./images/operator/".$data1[$i])) {
					unlink("./images/operator/".$data1[$i]);
				}
			}
			$pesan = "a";
			json_encode($pesan);
